#!/bin/bash
#
# Styxy PreToolUse Hook - Storybook Interception
# Intercepts Storybook commands and allocates ports via Styxy daemon
#
...
# Logging function
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [PreToolUse-Storybook] $*" >> "${HOOK_LOG_FILE}"
}
...
# Check if command contains Storybook
is_storybook_command() {
    local args="$1"
    if echo "${args}" | grep -qE "\bstorybook\b"; then
        return 0
    fi
    return 1
}
...
extract_existing_port() {
    ...
    # Check for various Storybook port patterns
    ...
    if response=$(curl -s --max-time 10 -X POST "${STYXY_URL}/allocate" \
        ...
        if echo "${response}" | grep -q '"success":true'; then
            ...
            if [[ -n "${allocated_port}" && -n "${lock_id}" ]]; then
                log "Allocated port ${allocated_port} for Storybook (lock: ${lock_id})"
...
main() {
    ...
    # Check if this is a Storybook command
    if ! is_storybook_command "${TOOL_ARGS}"; then
        log "Not a Storybook command, allowing execution"
        create_hook_output "allow" "Not a Storybook command" ""
        return 0
    fi

    log "Detected Storybook command, attempting port allocation"
    ...
    if allocation_result=$(allocate_port "${existing_port}" "${instance_id}"); then
        ...
        log "Successfully modified Storybook command with port ${allocated_port}"
